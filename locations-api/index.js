...
async function geocodeAddress(address) {
    const { data } = await axios.get(`https://nominatim.openstreetmap.org/search.php?q=${address}&polygon_geojson=1&format=jsonv2`);
    if (!data.length) {
        return {
            latitude: 0,
            longitude: 0,
...
async function getPlaces() {
    ...
    await fs.promises.writeFile('places.json', JSON.stringify(places, null, 2), 'utf');
    console.log('DONE!');
}
